...
### Dimensionality Reduction: If you have 100 features, PCA compresses them into a few principal components while keeping most of the information (variance).
### Data Visualization: Humans can't visualize 10D or 100D space. PCA reduces the data to 2D or 3D so you can plot it and look for natural clusters, patterns, or extreme outliers.
### Pre-processing for Clustering: Algorithms like K-Means struggle in high dimensions due to the "curse of dimensionality" (distances between points start to look the same). Running PCA before K-Means helps the clustering algorithm perform much better and run faster.
