...
PCA can be used across all three paradigms, but its role changes depending on whether you are working with supervised or unsupervised data.
...
## Why do it? Much like in classification, if your independent variables are highly correlated (e.g., a house's "square footage" and "number of rooms"), standard Linear Regression estimates become highly unstable. PCR solves this by making the predictors completely independent of one another.

## Note: The Golden Rule: Don't Leak Data!

When using PCA for supervised learning (Classification or Regression), always fit the PCA transformer on the training set only, and then transform both the training and testing sets. If you run PCA on the entire dataset before splitting, information from the test set will "leak" into your training data, giving you overly optimistic (and fake) performance metrics.
