...
A standard neural network processes inputs independently and cannot remember previous words. Also, input size is fixed in ANN and CNN.

RNN solves this by maintaining a hidden state (memory) and input size can vary.

---

# Basic Architecture of RNN

<img width="700" height="400" alt="image" src="https://github.com/user-attachments/assets/dd4c37b1-6d40-4fed-ac6a-d1415a5116ea" />


* Time Step $t-1$: Processes the first input ($x_{t-1}$) and creates hidden state $h_{t-1}$.
* Time Step $t$: Takes the current input ($x_t$) AND the previous hidden state ($h_{t-1}$) to calculate the new hidden state ($h_t$) and output ($y_t$).
* Time Step $t+1$: Repeats the process, passing $h_t$ forward.
...
Note: Weight at each time step remain same. Weight only changes when back propagation is done to reduce the loss.
...
<img width="343" height="50" alt="image" src="https://github.com/user-attachments/assets/52852535-d145-47aa-957c-ac7308a9020b" />

### Output

<img width="216" height="50" alt="image" src="https://github.com/user-attachments/assets/e69c7ecc-d614-45ab-bc6f-bff41f3ad543" />
